...
namespace HC_WEB_FINALPROJECT.Controllers {
    public class HomeController : Controller {
        ...
        public IActionResult Dashboard () {
            ...
            var Female = from a in _AppDbContext.Employee where a.Gender == "female"
            select a;
            var Male = from a in _AppDbContext.Employee where a.Gender == "male"
            select a;
            var Applicant = from a in _AppDbContext.Applicant where a.Status_Proccess == "unproccess"
            select a;
            var ApplicantView = (from a in _AppDbContext.Applicant where a.Status_Proccess == "unproccess"
                select a).Take (4);
